# IDM 延长脚本
一个开源工具，用于延长和重置试用期 [Internet Download Manager](https://www.internetdownloadmanager.com/)

# 免责声明
我想澄清一下，我不是这个脚本的原作者。我创建这个仓库的主要目的是为中文用户简化流程。此外，作为对脚本原作者工作的尊重，我确保注明了他们。

# 特点
* IDM 冻结试用及使用注册表键锁定方法进行延长
* 即使安装了 IDM 更新，延长和试用仍然存在。
* IDM 试用重置
* 完全开源
* 基于透明批处理脚本
...
[GitHub](https://github.com/Cotton059/IDM-Activation-Script)
...
# 日志
* 📌 该延长选项目前在脚本中无法使用，请使用 “冻结试用” 选项将 30 天的试用期锁定为终身有效。
...
* 从 [GitHub](https://github.com/Cotton059/IDM-Activation-Script/archive/refs/heads/main.zip)下载该文件
* 右键点击下载的压缩文件并解压
* 右键点击下载的压缩文件并解压 `IAS.cmd`
* 你将看到延长选项，并按照屏幕上的说明操作。
* 就这些。
...
* 此脚本应用注册表锁定方法来延长互联网下载管理器（IDM）。
* 此方法在延长时需要联网。
* IDM更新可以直接安装，无需再次延长。
* 延长后，如果在某些情况下，IDM开始显示延长提示屏幕，那么只需再次运行延长选项，无需使用重置选项。

## 重置IDM延长/试用
* 互联网下载管理器提供30天的试用期，你可以随时使用此脚本重置延长/试用期。
...
## 高级信息
* 要在无人参与模式下延长，请使用 /act 参数运行脚本。
* 要在无人值守模式下冻结试验，请使用 /frz 参数运行脚本。
* 要在无人参与模式下重置，请使用/res参数运行脚本。

# 它是如何工作的？
* IDM会在各种注册表项中存储与试用和延长相关的数据。其中一些注册表项被锁定，以防止篡改，并且数据以一种特定模式存储，用于追踪伪造序列号问题和剩余试用天数。要延长它，此处的脚本只需通过在IDM中触发一些下载来生成这些注册表项，识别这些注册表项并将其锁定，这样IDM就无法编辑和查看它们。通过这种方式，IDM就不会显示它是使用伪造序列号延长的警告。 
...
# 变更日志
## v1.2
* 添加了带有随机生成的姓名、电子邮件和注册详细信息中的密钥的延长选项，并附带警告，称该选项对部分用户不起作用，建议的选项是使用冻结试用。
## v1.1
* IDM 6.42b3版本更新后，在使用IAS延长时开始出现虚假的序列号弹窗。因此，我们移除了延长选项，取而代之的是“冻结试用”选项，以便终身锁定30天的试用期。 
...
## v0.9
* 修复了一个脚本在非管理员用户账户中无法延长和重置IDM的问题。
* 修复了一个脚本错误显示IDM已延长的问题。
* 修复了可能出现虚假序列号弹窗的问题。该脚本还将显示信息，以便在不使用重置选项的情况下再次运行延长选项。
...
* 增加了许多错误检查，以便更好地识别问题。
## v0.8
* 将项目转移到 [Github](https://github.com/Cotton059/IDM-Activation-Script)
* 小错误修复
...
|                                             |                                                              |
| ------------------------------------------- | ------------------------------------------------------------ |
| Dukun Cabul                                 | 此IDM试用重置和延长逻辑的最初研究者，为这些方法制作了一款Autoit工具, [IDM-AIO_2020_Final](https://nsaneforums.com/topic/371047-discussion-internet-download-manager-fixes/page/8/#comment-1632062) |
